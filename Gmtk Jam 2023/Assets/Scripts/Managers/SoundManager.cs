...
namespace Game.Managers
{
    ...
    public class SoundManager : Singleton<SoundManager>
    {
        ...
        {
            //musicAudioSource.clip = inGame ? backgroundMusicTwo : backgroundMusicOne;
        ...
        private bool CanPlaySound(SoundType sound)
        {
            switch (sound)
            {
                ...
                case SoundType.Laser:
                    ...
                    {
                        float lastTimePlayed = _soundTimerDictionary[sound];
                        const float laserTimerMax = .1f;
                        if (!(lastTimePlayed + laserTimerMax < Time.time))
                            return false;
    ...
    {
        PlayerWalk, Hit, Explosion, Damage, Blop, ButtonClicked, FireBall, Purchase, FireEffect, LevelUp, Reroll, Error,
        BossDeath, Victory, WarriorHitSound, FighterHitSound, SwordSwipe, Laser, SwordCreated, Lose, PlayerHit
    }
}
