	...
	public class Player : MonoBehaviour
	{
		...
		private void GetInteractionInput()
		{
			...
			if (_currentInteractable.GetInteractableType() == InteractableType.Item)
			{
				...
				SoundManager.Instance.PlaySound(SoundType.RecogerItem);
			...
			else
			{
				...
				{
					SoundManager.Instance.PlaySound(SoundType.DejarItem);
		...
		private void Move()
		{
			bool walkCondition = Mathf.Abs(_horizontal) > 0.1f || Mathf.Abs(_vertical) > 0.1f;
			animator.SetBool(Walking, walkCondition);
			if (walkCondition)
